...
$args = $_G['cache']['plugin']['csdn123com_toutiao']['csdn123_uid'];
...
if (in_array($_G['uid'], $argsArr)) {
	...
	$chk = DB::fetch_first("SELECT ID,tid_aid,forum_portal FROM " . DB::table('csdn123toutiao_news') . " WHERE source_link='" . $fromurl . "'");
    ...
    if ($recode == 'ok') {
        $chk = DB::fetch_first("SELECT ID,tid_aid,forum_portal FROM " . DB::table('csdn123toutiao_news') . " WHERE ID=" . intval($ID));
        if ($chk['tid_aid'] > 0) {
            echo preview_url($chk['forum_portal'], $chk['tid_aid']);
        } else {
            echo "no2";
