...
$server_url = 'action=plugins&operation=config&do=' . $pluginid . '&identifier=csdn123com_toutiao&pmod=adminVest';
if ($_GET['formhash'] == FORMHASH && empty($_GET['deluser']) == false && $_GET['deluser'] == 'yes') {
	
	if(empty($_GET['clears_all'])==false)
	{
		DB::delete('csdn123toutiao_reguser', 'uid>0');
	...
	include template('csdn123com_toutiao:delete_vest_list');
	...
	require './source/plugin/csdn123com_toutiao/regvest.inc.php';	

} elseif($_GET['formhash'] == FORMHASH && !empty($_GET['subpmod']) && $_GET['subpmod']=='importVest') {
	
	require './source/plugin/csdn123com_toutiao/importVest.inc.php';
	
} elseif($_GET['formhash'] == FORMHASH && !empty($_GET['subpmod']) && $_GET['subpmod']=='exportVest') {
	
	require './source/plugin/csdn123com_toutiao/exportVest.inc.php';
	...
	$VestCount = DB::result_first('SELECT count(*) FROM ' . DB::table('csdn123toutiao_reguser'));
	if($VestCount>0)
	{
	...
	include template('csdn123com_toutiao:adminVest');
