...
$server_url = 'action=plugins&operation=config&do=' . $pluginid . '&identifier=csdn123com_toutiao&pmod=adminVest&subpmod=importVest&formhash=' . FORMHASH;
if ($_GET['formhash'] == FORMHASH && empty($_GET['inputuser']) == false && $_GET['inputuser'] == 'yes') {
	...
	$back_server_url = 'action=plugins&operation=config&do=' . $pluginid . '&identifier=csdn123com_toutiao&pmod=adminVest';
	cpmsg("csdn123com_toutiao:import_vest_success", $back_server_url, "succeed");
}

include template('csdn123com_toutiao:importVest');
