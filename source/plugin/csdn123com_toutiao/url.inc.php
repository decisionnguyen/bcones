...
$server_url='action=plugins&operation=config&do=' . $pluginid . '&identifier=csdn123com_toutiao&pmod=url';
...
if($_GET['formhash'] == FORMHASH && !empty($_GET['csdn123_agrs']) && $_GET['csdn123_agrs']=='yes')
{
	...
	{
		cpmsg("csdn123com_toutiao:content_url_error","","error");
	} else {
	...
	$chk = DB::fetch_first("SELECT * FROM " . DB::table('csdn123toutiao_news') . " WHERE source_link='" . daddslashes($content_url) . "' LIMIT 1");
	if (count($chk) == 0)
	{
		$newsArr=array();
		...
		$ID = DB::insert('csdn123toutiao_news', $newsArr,true);
	}
	...
	if($recode=='ok')
	{
		$succeed_url='action=plugins&operation=config&do=' . $pluginid . '&identifier=csdn123com_toutiao&pmod=released';
	...
	include template('csdn123com_toutiao:url');
