...
$server_url = 'action=plugins&operation=config&do=' . $pluginid . '&identifier=csdn123com_toutiao&pmod=pending';
if ($_GET['formhash'] == FORMHASH && $_GET['selall'] == 'yes') {
	...
	if (empty($_GET['selimport']) == false) {
		...
		if (is_numeric($ID) == false) {
			$success_Url = 'action=plugins&operation=config&do=' . $pluginid . '&identifier=csdn123com_toutiao&pmod=released';
			cpmsg('csdn123com_toutiao:succeed', $success_Url, 'succeed');
		}
		...
		if ($recode == 'ok') {
			...
			$statusStr = lang('plugin/csdn123com_toutiao', 'import_status');
			...
			$nextCatchUrl = '?' . $server_url . '&selimport=yes&selall=yes&formhash=' . FORMHASH . '&idarray=' . $idstr . '&num=' . $num;
			$statusStr = lang('plugin/csdn123com_toutiao', 'import_status_error');
			$statusStr = str_replace('num', $num, $statusStr);
	...
	$thread = dfsockopen("http://discuz.csdn123.net/catch/toutiao201711/random_new.php");
	...
	if(is_array($thread))
	{
		...
		cpmsg("csdn123com_toutiao:collection_success",$server_url,"succeed");
	}	
	...
	include template("csdn123com_toutiao:pending_randNews");
	...
	if ($recode == 'ok') {
		...
		cpmsg('csdn123com_toutiao:fail', '', 'error');
	...
	require_once libfile('function/group');
	$grouplistArr=grouplist('displayorder',false,100);
	include template("csdn123com_toutiao:pending_update");
	...
	cpmsg('csdn123com_toutiao:succeed', $update_backUrl, 'succeed');
	...
	DB::delete('csdn123toutiao_news','tid_aid<=0');
	cpmsg('csdn123com_toutiao:succeed', $server_url, 'succeed');
	...
	if($newsCount>0)
	{
		...
		$news_list = DB::fetch_all('SELECT * FROM ' . DB::table('csdn123toutiao_news') . ' WHERE tid_aid<=0 and del=0 ORDER BY ID DESC LIMIT ' . $startNum . ',20');
		$start = $page - 4;		
	...
	include template("csdn123com_toutiao:pending");
