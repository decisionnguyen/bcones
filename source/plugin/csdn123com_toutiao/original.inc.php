...
$server_url = 'action=plugins&operation=config&do=' . $pluginid . '&identifier=csdn123com_toutiao&pmod=original';
if ($_GET['formhash'] == FORMHASH && empty($_GET['weiyanchang_add']) == false && $_GET['weiyanchang_add'] == 'show') {
	
	include template('csdn123com_toutiao:original_add');
	
} elseif ($_GET['formhash'] == FORMHASH && empty($_GET['weiyanchang_import']) == false && $_GET['weiyanchang_import'] == 'show') {
	
	include template('csdn123com_toutiao:original_import');
	...
	$autoImportDataUrl = "http://www.csdn123.net/zd_version/zd9/original_data/{$n}.php?" . urlencode($_G['siteurl']);
	...
	{
		cpmsg('csdn123com_toutiao:data_wrong_format', '', 'error');
	}
	...
	foreach($autoImportDataArr['word'] as $word_value)
	{
		...
		$wordSql="REPLACE INTO " . DB::table('csdn123toutiao_weiyanchang') . "(word1,word2) VALUES('" . $word1 . "','" . $word2 . "')";
		DB::query($wordSql);
	}
	if($n>=6)
	{
	...
	$next_original_url = 'action=plugins&operation=config&do=' . $pluginid . '&identifier=csdn123com_toutiao&pmod=original&weiyanchang_auto_import=yes&formhash=' . FORMHASH . '&n=' . $n;
	$original_autoimport_info = lang('plugin/csdn123com_toutiao', 'original_autoimport_info');
	...
	include template('csdn123com_toutiao:original_output');
	...
	cpmsg('csdn123com_toutiao:succeed',$server_url . '&page=' . $_GET['page'], 'succeed');
	...
	include template('csdn123com_toutiao:original_modify');
	...
	cpmsg('csdn123com_toutiao:succeed', $server_url . '&page=' . $page, 'succeed');
	...
	$page=max(1,$page);
	$weiyanchangCount = DB::result_first('SELECT count(*) FROM ' . DB::table('csdn123toutiao_weiyanchang'));
	if($weiyanchangCount>0)
	{
		$TotalNumber = lang('plugin/csdn123com_toutiao', 'total_number');
		...
		$weiyanchang_list = DB::fetch_all('SELECT * FROM ' . DB::table('csdn123toutiao_weiyanchang') . ' ORDER BY ID DESC LIMIT ' . $startNum . ',20');
		$start = $page - 4;		
	...
	include template('csdn123com_toutiao:original');
