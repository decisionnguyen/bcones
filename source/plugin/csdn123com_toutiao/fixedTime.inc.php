...
$server_url='action=plugins&operation=config&do=' . $pluginid . '&identifier=csdn123com_toutiao&pmod=fixedTime';
...
if($_GET['formhash'] == FORMHASH && !empty($_GET['csdn123_agrs']) && $_GET['csdn123_agrs']=='yes')
{
	...
	if(empty($_GET['update']))
	{
		...
		DB::update('csdn123toutiao_cron',$cronArr,'ID=' . $news_id);
	}
	$succeed_url='action=plugins&operation=config&do=' . $pluginid . '&identifier=csdn123com_toutiao&pmod=fixedTime';
	cpmsg("csdn123com_toutiao:succeed",$succeed_url,"succeed");
	...
	$id = intval($_GET['del']);
	DB::delete('csdn123toutiao_cron','ID=' . $id);
	cpmsg("csdn123com_toutiao:succeed",$server_url,"succeed");
	...
	$update_rs=DB::fetch_first("SELECT * FROM " . DB::table('csdn123toutiao_cron') . " WHERE ID=" . $update_id);
	...
	include template('csdn123com_toutiao:fixedTimeUpdate');

} elseif(!empty($_GET['clears_all']) && $_GET['clears_all']=='yes' && $_GET['formhash'] == FORMHASH) {
	
	DB::delete('csdn123toutiao_cron','ID>0');
	cpmsg('csdn123com_toutiao:succeed', $server_url, 'succeed');
	...
	include template('csdn123com_toutiao:fixedTimeAdd');
	...
	$hzw_startcron = $_G['cache']['plugin']['csdn123com_toutiao']['csdn123_dingshi'];
	if($hzw_startcron != 1)
	{
		echo '<div style="text-align:center;margin:64px;"><a href="?action=plugins&operation=config&do=' . $pluginid . '" style="font-size:24px;color:red;">' . lang('plugin/csdn123com_toutiao', 'open_timing_acquisition') . '</a></div>';
		...
		$pendingUrl='action=plugins&operation=config&do=' . $pluginid . '&identifier=csdn123com_toutiao&pmod=pending';
		$cronList=DB::fetch_all("SELECT * FROM " . DB::table('csdn123toutiao_cron') . " ORDER BY catchtime DESC");
		$pending_count = lang('plugin/csdn123com_toutiao', 'pending_count');
		$newsCount = DB::result_first('SELECT count(*) FROM ' . DB::table('csdn123toutiao_news') . ' WHERE tid_aid<=0 and del=0');
		$pending_count = str_replace('x',$newsCount,$pending_count);
		include template('csdn123com_toutiao:fixedTime');
