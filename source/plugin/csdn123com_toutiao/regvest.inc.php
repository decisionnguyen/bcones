...
$server_url = 'action=plugins&operation=config&do=' . $pluginid . '&identifier=csdn123com_toutiao&pmod=adminVest&subpmod=regvest&formhash=' . FORMHASH;
if ($_GET['formhash'] == FORMHASH && empty($_GET['adduser']) == false && $_GET['adduser'] == 'yes') {
	...
	{
		cpmsg("csdn123com_toutiao:reg_err", '', "error");
	}
	...
	foreach ($regData as $regValue) {
		...
		DB::insert('csdn123toutiao_reguser', $addUser);
	...
	cpmsg("csdn123com_toutiao:registered_successfully", $server_url, "succeed");
	...
	include template('csdn123com_toutiao:regvest');
