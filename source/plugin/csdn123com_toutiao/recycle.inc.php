...
$server_url = 'action=plugins&operation=config&do=' . $pluginid . '&identifier=csdn123com_toutiao&pmod=recycle';
if ($_GET['formhash'] == FORMHASH && empty($_GET['canceldel']) == false) {
	...
	cpmsg('csdn123com_toutiao:succeed', $canceldel_url, 'succeed');
	...
	cpmsg('csdn123com_toutiao:succeed', $del_backurl, 'succeed');
	...
	if (empty($_GET['restoreall']) == false) {
		...
		DB::update('csdn123toutiao_news',array('del'=>0),'del=1');
		cpmsg('csdn123com_toutiao:succeed', $server_url, 'succeed');
	...
	$page=max(1,$page);
	$newsCount = DB::result_first("SELECT COUNT(*) FROM " . DB::table('csdn123toutiao_news') . " WHERE tid_aid=0 and del=1");
	if($newsCount>0)
	{
		...
		$news_list = DB::fetch_all('SELECT * FROM ' . DB::table('csdn123toutiao_news') . ' WHERE tid_aid=0 and del=1 ORDER BY ID DESC LIMIT ' . $startNum . ',20');
		$start = $page - 4;		
	...
	include template("csdn123com_toutiao:recycle");	
