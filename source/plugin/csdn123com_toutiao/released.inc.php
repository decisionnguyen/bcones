...
$server_url = 'action=plugins&operation=config&do=' . $pluginid . '&identifier=csdn123com_toutiao&pmod=released';
...
if ($_GET['formhash'] == FORMHASH && empty($_GET['clears_all']) == false && $_GET['clears_all'] == 'yes') {
	...
	{
		$count_num = DB::result_first('SELECT count(*) FROM ' . DB::table('csdn123toutiao_news') . ' WHERE tid_aid>0');		
	} else {		
	...
	if(empty($_G['cache']['plugin']['csdn123com_toutiao']['csdn123_seo']))
	{
		cpmsg('csdn123com_toutiao:baidu_seo_error', '', 'error');
	} else {
		$csdn123_seo=$_G['cache']['plugin']['csdn123com_toutiao']['csdn123_seo'];
	}
	$post_seo_url_RS=DB::fetch_first("SELECT * FROM " . DB::table('csdn123toutiao_news') . " WHERE tid_aid>0 AND ID>" . $id . " ORDER BY ID ASC LIMIT 1");
	...
	$page=max(1,$page);
	$newsCount = DB::result_first('SELECT count(*) FROM ' . DB::table('csdn123toutiao_news') . ' WHERE tid_aid>0');
	if($newsCount>0)
	{
		...
		$news_list = DB::fetch_all('SELECT * FROM ' . DB::table('csdn123toutiao_news') . ' WHERE tid_aid>0 ORDER BY send_datetime DESC LIMIT ' . $startNum . ',20');
		$start = $page - 4;		
	...
	include template("csdn123com_toutiao:released");
