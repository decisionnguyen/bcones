...
if($count) {
	...
	$articles = array();
	$favoriteList = C::t('home_favorite')->fetch_all_by_uid_idtype($_G['uid'], $idtype, $favid, $start,$perpage);
	if(isset($_GET['fup'])){
       $fids =C::t('forum_forum')-> get_all_group_fids( $_GET['fup'] );
       $uid = isset($_GET['uid'])? $_GET['uid']:$_G['uid'];
       $favoriteList = C::t('home_favorite')->fetch_all_by_uid_id($uid, $fids, $idtype = 'fid',$favid, $start , $perpage);
    }
