...
class table_forum_forum extends discuz_table
{
    ...
    public function get_all_group_fids($fup ,$allstatus = 0, $type = 'forum') {
        $typesql = DB::field('type', $type);
        $statussql = empty($allstatus) ? ' AND status<>3' : '';
        $fupsql = ' AND '.DB::field('fup', $fup);
        $query = "SELECT * FROM ".DB::table($this->_table)." WHERE $typesql $statussql $fupsql";
        $list = DB::fetch_all($query,[],'fid');
